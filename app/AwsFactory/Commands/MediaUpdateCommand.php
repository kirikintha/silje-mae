...
class MediaUpdateCommand extends Command {
	...
	private function rotate() {
		if (isset($this->video['video']['tags']['rotate'])) {
			...
			switch ($rotation) {
				...
				case 90:
				default:
					$command = sprintf('ffmpeg -y -i %s -metadata:s:v rotate="0" -vf "transpose=1" -acodec copy %s', $this->video['in'], $rotated);
					break;
			}
			//Rotate
			$this->exec($command);
	...
	private function makeFLV($file_name, $extension) {
		...
		//Exec FFMEG rotation.
		$command = sprintf('ffmpeg -y -i %s -c:v libx264 -crf 19 %s', $this->video['in'], $this->video['out']);
		$this->exec($command);
		$this->video['created'][] = $this->video['out'];
		$this->putVideo($this->video['out']);
	}
